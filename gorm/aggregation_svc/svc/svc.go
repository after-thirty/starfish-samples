...
func (svc *Svc) CreateSoBatch(ctx context.Context, rollback bool) error {
	rootContext := ctx.(*context2.RootContext)
	soMasters := []*dao.SoMaster{
		{
			BuyerUserSysno:       10001,
	...
	soReq, err := json.Marshal(q1)
	fmt.Println(string(soReq))
	req1, err := http.NewRequest("POST", "http://localhost:8002/deleteBatchSO", bytes.NewBuffer(soReq))
	if err != nil {
		panic(err)
	}
	req1.Header.Set("Content-Type", "application/json")
...
type ProxyService struct {
	*Svc
	...
	CreateSoBatch func(ctx context.Context, rollback bool) error
}
...
func init() {
	...
	methodTransactionInfo["CreateSoBatch"] = &tm.TransactionInfo{
		TimeOut:     60000000,
		Name:        "CreateSoBatch",
		Propagation: tm.REQUIRED,
	}
